...
import "../protocols/HappyStake.sol";

contract HappyStakeAdapter is ReentrancyGuard {
    IERC20 public immutable stakeToken;
    IERC20 public immutable rewardToken;
    HappyStake public immutable happyStake;
    ...
    constructor(address _happyStake, address _aggregator) {
        happyStake = HappyStake(_happyStake);
        stakeToken = IERC20(happyStake.stakingToken());
        rewardToken = IERC20(happyStake.rewardToken());
    ...
    /**
     * @notice Deposit tokens into HappyStake via adapter with 1% platform fee.
    ...
    function deposit(uint256 amount, address user) external nonReentrant onlyAggregator {
        ...
        stakeToken.approve(address(happyStake), stakeAmount);
        happyStake.stakeTokens(user, stakeAmount); 

        emit Deposited(user, stakeAmount);
    }

    /**
     * @notice Withdraw staked tokens from HappyStake via adapter.
    ...
    function withdraw(uint256 amount, address user) external nonReentrant onlyAggregator {
        happyStake.unstakeTokens(user, amount); 
        emit Withdrawn(user, amount);
    }

    /**
     * @notice Claim rewards for a user (pass-through to HappyStake).
     * @param user User address to send the rewards.
     */
    function claimRewards(address user) external nonReentrant onlyAggregator {
        happyStake.claimReward(user); 
    }
    ...
    function withdrawAll(address user) external nonReentrant onlyAggregator {
        uint256 amount = happyStake.stakedBalance(user); // 直接查詢 HappyStake 的質押餘額
        if (amount > 0) {
            happyStake.unstakeTokens(user, amount);
            emit Withdrawn(user, amount);
        }
    ...
    function getStakedBalance(address user) external view returns (uint256) {
        return happyStake.stakedBalance(user);
    }
    ...
    function getCurrentBalance(address user) external view returns (uint256) {
        return happyStake.stakedBalance(user) + happyStake.pendingReward(user);
    }
    ...
    function getPendingRewards(address user) external view returns (uint256) {
        return happyStake.pendingReward(user);
    }
}